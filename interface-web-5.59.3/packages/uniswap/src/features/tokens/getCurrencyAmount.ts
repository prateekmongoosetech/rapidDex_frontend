...
export function getCurrencyAmount<T extends Currency>({
  ...
  try {
    ...
    if (valueType === ValueType.Exact) {
      // Truncate to token precision to prevent "fractional component exceeds decimals" error
      // when a high-precision amount (e.g. from an 18-decimal token) is used with a lower-decimal token (e.g. WBTC=8)
      const dotIndex = parsedValue.indexOf('.')
      if (dotIndex !== -1 && parsedValue.length > dotIndex + 1 + currency.decimals) {
        parsedValue = parsedValue.slice(0, dotIndex + 1 + currency.decimals)
      }
