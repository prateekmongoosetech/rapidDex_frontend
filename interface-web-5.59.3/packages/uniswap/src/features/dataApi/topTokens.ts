...
import { getMstCurrencyInfos, isMstDexChain } from 'uniswap/src/features/tokens/mstTokenList'
...
export function usePopularTokens(chainFilter: UniverseChainId): GqlResult<CurrencyInfo[]> {
  ...
  const useLocalMstList = isMstDexChain(chainFilter)
  ...
  const { data, loading, error, refetch } = useTopTokensQuery({
    ...
    skip: isTestnet || useLocalMstList,
  })
  ...
  return useMemo(
    () => {
      if (useLocalMstList) {
        return { data: getMstCurrencyInfos(), loading: false, error: undefined, refetch: () => {} }
      }

      return { data: formattedData, loading, error: persistedError, refetch }
    },
    [formattedData, loading, persistedError, refetch, useLocalMstList],
  )
}
