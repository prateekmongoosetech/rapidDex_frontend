...
export function useTokenApprovalInfo(params: TokenApprovalInfoParams): TokenApprovalInfo & TokenApprovalGasInfo {
  ...
  return useMemo(() => {
    // MST Testnet: Trading API doesn't support this chain; assume no approval needed.
    // V3 SwapRouter uses direct ERC20 allowance (not Permit2) — users approve manually if needed.
    if (chainId === UniverseChainId.MstTestnet) {
      return { action: ApprovalAction.None, txRequest: null, cancelTxRequest: null, isLoading: false }
    }
  ...
  }, [chainId, activeGasStrategy, approvalRequestArgs, data, error, isWrap, isLoading])
}
