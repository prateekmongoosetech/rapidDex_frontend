...
export function fromGraphQLChain(chain: Chain | string | undefined): UniverseChainId | null {
  switch (chain) {
    ...
    case 'mst_testnet' as Chain:
    case 'UNKNOWN_CHAIN' as Chain:
      return UniverseChainId.MstTestnet
  }
...
export function getEnabledChains({
  ...
  // MST DEX: always return only MST Testnet regardless of testnet mode setting.
  // gqlChains is empty — Uniswap GraphQL Chain enum has no MST; balances/tokens use local/on-chain paths.
  return {
    chains: [UniverseChainId.MstTestnet],
    gqlChains: [],
    defaultChainId: UniverseChainId.MstTestnet,
    isTestnetModeEnabled: true,
  }
}

export function hasGraphqlBackendChains(gqlChains: GqlChainId[]): boolean {
  return gqlChains.length > 0
}
