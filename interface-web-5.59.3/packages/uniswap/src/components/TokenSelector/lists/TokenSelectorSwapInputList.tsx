...
import { isMstDexChain } from 'uniswap/src/features/tokens/mstTokenList'
...
function useTokenSectionsForSwapInput({
  ...
  const loading = isTestnetModeEnabled
    ? !commonTokenOptions
    : portfolioTokenOptionsLoading || popularTokenOptionsLoading || favoriteTokenOptionsLoading
  ...
  const showSuggestedTokens = chainFilter && (isTestnet || isMstDexChain(chainFilter))

  const suggestedSection = useTokenOptionsSection({
    sectionKey: TokenOptionSection.SuggestedTokens,
    tokenOptions: [(showSuggestedTokens ? commonTokenOptions : []) ?? []],
  })
  ...
  const sections = useMemo(() => {
    if (isSwapListLoading(loading, portfolioSection, popularSection, { isTestnetMode: isTestnetModeEnabled, suggestedSection })) {
      return undefined
    }
