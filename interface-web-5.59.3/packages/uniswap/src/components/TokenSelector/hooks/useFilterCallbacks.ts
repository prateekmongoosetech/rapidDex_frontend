...
import { getEffectiveChainFilter } from 'uniswap/src/features/tokens/mstTokenList'
...
export function useFilterCallbacks(
  ...
  const [chainFilter, setChainFilter] = useState<UniverseChainId | null>(() => getEffectiveChainFilter(chainId))
  ...
  useEffect(() => {
    setChainFilter(getEffectiveChainFilter(chainId))
  }, [chainId])
