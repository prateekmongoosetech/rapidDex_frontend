...
export function useCommonTokensOptions(
  ...
  const useLocalListOnly = isMstDexChain(chainFilter)

  return {
    data: filteredCommonBaseTokenOptions,
    refetch,
    error: error || undefined,
    loading: useLocalListOnly ? loadingCommonBaseCurrencies : loadingPorfolioBalancesById || loadingCommonBaseCurrencies,
  }
}
